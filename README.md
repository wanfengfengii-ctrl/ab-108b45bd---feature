...
定位系统发生系统性平移时，不必对已录入姿态逐一试错：在演练页填写一个**非负的统一附加定位误差**，
以当前所见修订号发起复核。该误差叠加到每个姿态自身的不确定半径上，仅针对**当时已部署支腿**形成的支承面，
为每个姿态求出**可放置投影中心的闭合区域**（支承凸包向内收缩有效半径后的半平面交集），并给出：

- 当前中心是否落入该区域（**边界接触仍安全**）；
- 距最近安全边界的有符号裕量（正为内净距，0 为相切，负为越界量）；
- 区域退化为线段/点或为空时的可解释结论。

复核是**只读**的，不写入事件轨迹；结果对应**提交时的修订号**。此后发生任何已接受操作，
旧结论不得冒充当前结论（过期修订号复核返回 `409`），人员可按最新修订号重新复核。
...
| POST | `/api/sessions/:id/safety-window` | 安全窗口复核 `{baseRevision, extraError}`（非负）；逐姿态返回闭合区域/内外判定/裕量/结论，只读不写轨迹 |

拒绝语义：`400` 请求非法 · `409` 修订号过期 · `422` 业务拒绝（圆盘越界 / 支承不足 / 无变化操作）。拒绝响应携带最新 `state`，且**不会**写入事件轨迹。复核结果携带其对应的 `revision`；修订号被新操作推进后，旧复核结论即失效，需重新复核。

## 架构

```
src/geometry.js   凸包（单调链）、圆盘-凸多边形包含（相切为安全）、支承评估、安全窗口（半平面内缩区域）
src/events.js     事件类型与纯函数 reducer；状态 = replay(events)
src/session.js    录入校验、操作试算、安全窗口复核、轨迹追加（内存 + 只增 JSONL）
